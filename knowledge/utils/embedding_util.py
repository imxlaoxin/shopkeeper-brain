...
@dataclass
class HybridVectorsRet:
    dense_vectors: List[List[float]]
    sparse_vectors: List[Dict[int, float]]

def generate_bge_m3_hybrid_vectors(model: BGEM3EmbeddingFunction, embedding_documents: List[str],is_query:bool=True) -> HybridVectorsRet:
    """
    为文本生成混合向量嵌入（稠密 + 稀疏）
    Args:
    ...
    # 6. 返回
    return HybridVectorsRet(
        dense_vectors=[den.tolist() for den in embedding_result["dense"]],
        sparse_vectors=processed_sparse
    )